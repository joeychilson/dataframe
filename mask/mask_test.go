...
func TestNewFuncPanicsForNegativeLength(t *testing.T) {
	defer func() {
		if recover() == nil {
			t.Fatal("NewFunc(-1, fn) did not panic")
		}
	}()
	NewFunc(-1, func(int) bool { return false })
}
...
func TestNegativeLengthPanics(t *testing.T) {
	tests := []struct {
		...
		{name: "All", call: func() { All(-1) }},
		{name: "None", call: func() { None(-1) }},
	}
...
func TestLogicalOperations(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "And", result: left.And(right), want: func(i int) bool { return i%2 == 0 && i%3 == 0 }},
		{name: "Or", result: left.Or(right), want: func(i int) bool { return i%2 == 0 || i%3 == 0 }},
		{name: "Not", result: left.Not(), want: func(i int) bool { return i%2 != 0 }},
	}
...
func TestLogicalOperationsPanicOnLengthMismatch(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "And", call: func() { short.And(long) }},
		{name: "Or", call: func() { short.Or(long) }},
	}
...
func BenchmarkRows(b *testing.B) {
	...
	for i := 0; i < length; i += bitsPerWord {
		sparseValues[i] = true
	}
	benchmarks := []struct {
		name string
